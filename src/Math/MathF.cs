...
public static class MathF
{
	...
	/// <summary>
	/// Takes an angle measured in radians.
	/// </summary>
	...
	{
		return AngleBetweenVectors(@from, to) * System.Math.Sign(@from.X * to.Y - @from.Y * to.X);
	}
	...
	public static float MoveTowards(float current, float target, float maxDelta)
	{
		...
		return System.Math.Abs(target - current) <= maxDelta ? target : current + System.Math.Sign(target - current) * maxDelta;
	}
	...
	public static float DeltaMoveTowards(float current, float target, float maxDelta)
	{
		...
		var offset = System.Math.Abs(target - current);
		var sign = System.Math.Sign(target - current);
		if (offset < maxDelta)
		{
			return offset * sign;
		}
	...
	/// <summary>
	/// Loops t from 0 if t is greater than length, e.g Loop(4, 3) => 1
	/// </summary>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static float Loop(float t, float length)
	{
		var numLoops = Floor(t / length);
		return System.Math.Clamp(t - numLoops * length, 0.0f, length); // Clamp may be overkill
	}
	...
	/// <summary>
	/// Lerp between two angles measured in radians
	/// </summary>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
