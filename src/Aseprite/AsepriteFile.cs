...
/// <summary>https://github.com/aseprite/aseprite/blob/main/docs/ase-file-specs.md 2021-09-28</summary>
public class AsepriteFile
{
    ...
    public static byte BYTE(BinaryReader reader)
    {
        return reader.ReadByte();
    }

    public static ushort WORD(BinaryReader reader)
    {
        return reader.ReadUInt16();
    }

    public static short SHORT(BinaryReader reader)
    {
        return reader.ReadInt16();
    }

    public static uint DWORD(BinaryReader reader)
    {
        return reader.ReadUInt32();
    }

    public static long LONG(BinaryReader reader)
    {
    ...
    public static byte[] BYTES(BinaryReader reader, int numberOfBytes)
    {
        return reader.ReadBytes(numberOfBytes);
    }

    public static string STRING(BinaryReader reader)
    {
        return Encoding.UTF8.GetString(BYTES(reader, WORD(reader)));
    }
...
#pragma warning restore 618
    ...
    public class Cel : IHasUserData
    {
        ...
        private uint[] ConvertBytesToPixels(Span<byte> reader, ColorDepth colorDepth)
        {
            ...
            if (colorDepth == ColorDepth.Grayscale)
            {
                return ReadGrayscale(reader);
            }

            if (colorDepth == ColorDepth.Indexed)
            {
